...
def main():
    ...
    results_save_path = os.path.join('./save_dir/' + args.dataset + '/' + args.task + '/results/transformer',
                                     'split' + split,
                                     args.input_type)
    ...
    if args.predict:
        ...
        ckpt_files = sorted([f for f in os.listdir(model_save_path) if f.startswith('checkpoint') and f.endswith('.ckpt')])

        for ckpt_file in ckpt_files:
            model_path = os.path.join(model_save_path, ckpt_file)
            print("Predict with ", model_path)

            mean_acc = torch.tensor(0.0)
            for obs_p in obs_perc:
                model.load_state_dict(torch.load(model_path))
                model.to(device)
                acc = predict(model, video_test_list, args, obs_p, n_class, actions_dict, device)
                mean_acc += acc  
            
            mean_acc /= len(obs_perc)  

            print(f"Mean accuracy for {ckpt_file}: {mean_acc}")

            if mean_acc > max_mean_acc:
                max_mean_acc = mean_acc
                best_ckpt = ckpt_file
                print(f"New best checkpoint: {best_ckpt} with mean_acc: {max_mean_acc}")

        print(f"Best checkpoint: {best_ckpt} with max mean_acc: {max_mean_acc}")
