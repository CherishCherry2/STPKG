...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward_post(self,
        ...
        # # FD-1
        src2,weight = self.self_attn(q, k, value=v, attn_mask=src_mask,key_padding_mask=src_key_padding_mask)
        weight = self.dropout(weight)
        v = v.transpose(0, 1)
        src2 = torch.bmm(weight,v)
        src2 = src2.permute(1, 0, 2)

        src = src + self.dropout1(src2)
        src = self.norm1(src)
        src2 = self.linear2(self.dropout(self.activation(self.linear1(src))))      #FD-2
        src = src + self.dropout2(src2)
        src = self.norm2(src)
        # src = self.dropout(src)   #FD-4
        return src
    ...
    def forward_pre(self, src,
        ...
        src = src + self.dropout1(src2)
        src2 = self.norm2(src)
        src2 = self.linear2(self.dropout(self.activation(self.linear1(src2))))   # FD-2
        src = src + self.dropout2(src2)
        # src = self.dropout(src)  # FD-4
        return src
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_post(self, tgt, memory,
        ...
        # # FD-1
        tgt2,weight = self.self_attn(q, k, value=v, attn_mask=tgt_mask,key_padding_mask=tgt_key_padding_mask)
        weight = self.dropout(weight)
        v = v.transpose(0, 1)
        tgt2 = torch.bmm(weight,v)
        ...
        tgt = self.norm2(tgt)
        tgt2 = self.linear2(self.dropout(self.activation(self.linear1(tgt))))    #FD-2
        tgt = tgt + self.dropout3(tgt2)
        tgt = self.norm3(tgt)
        # tgt = self.dropout(tgt)  # FD-4
        return tgt
    ...
    def forward_pre(self, tgt, memory,
        ...
        tgt = tgt + self.dropout2(tgt2)
        tgt2 = self.norm3(tgt)
        tgt2 = self.linear2(self.dropout(self.activation(self.linear1(tgt2)))) # FD-2
        tgt = tgt + self.dropout3(tgt2)
        # tgt = self.dropout(tgt)    # FD-4
        return tgt
